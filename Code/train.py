...
if not flag_eval_only:
    full_list = load_list(dir_dataset + '/train.lst', dev_size)
    from sklearn.model_selection import train_test_split
    train_pdbs, valid_pdbs = train_test_split(
        full_list,
        test_size=0.3,
        random_state=42,
        shuffle=True
    )
    print(f"Train size: {len(train_pdbs)}, Valid size: {len(valid_pdbs)}")
    length_dict = {}
    for pdb in train_pdbs:
        try:
            (ly, seqy, cb_map) = np.load(
                f"{dir_dataset}/train/distance/{pdb}-cb.npy",
                allow_pickle=True
            )
            length_dict[pdb] = ly
        except:
    ...
    train_dataset = DistDataset(
        train_pdbs, all_feat_paths, all_dist_paths,
        training_window, pad_size, expected_n_channels,
        label_engineering='16.0'
    )

    valid_dataset = DistDataset(
        valid_pdbs, all_feat_paths, all_dist_paths,
        training_window, pad_size, expected_n_channels,
        label_engineering='16.0'
    )
    ...
    for epoch in range(training_epochs):
        ...
        train_loss = 0.0
        train_mae = 0.0
        with tqdm(total=len(train_loader), desc=f'Epoch {epoch + 1}/{training_epochs}',
                  unit='batch', position=0, leave=True) as pbar:
            for X_batch, Y_batch in train_loader:
                ...
                mae = mean_absolute_error(Y_batch_np.reshape(len(Y_batch_np), -1),
                                          output_np.reshape(len(output_np), -1))
    ...
    test_list = load_list(os.path.join(dir_dataset, 'CASP13.lst'))'''CASP14.lst,CASP14.lst,CASP15.lst'''
    valid_pdbs = []
