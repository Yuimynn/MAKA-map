'''
This code provides a set of functions for evaluating protein structure prediction results.
'''
'''
...
文件:重建三维模型与DISTFOLD使用预测的距离图和评估使用TM-score
构建模型的选项:
a)一个普通的2D numpy距离图
b) trRosetta预测距离图
c) CASP RR文件
...
def calc_dist_errors(P, Y, L, dist_thres=None, min_sep=None, top_l_by_x=None, pred_limit=None):
    ...
    for pair in sorted(pred_dict.items(), key=lambda x: x[1]):
        ...
        if xl == 0: break
    if len(pred_list) > 1:
        pred_list_np = np.array(pred_list)  # 转换为NumPy数组
        true_list_np = np.array(true_list)  # 转换为NumPy数组
