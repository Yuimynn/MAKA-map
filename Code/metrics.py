...
epsilon = 1e-7
# The predicted real-valued distance matrix. It calculates accuracy values, distance metrics, and Cb-LDDT scores.
# This function iterates through the predicted and actual values in a loop, computes various indicators, and outputs the results.
def evaluate_distances(PRED, YTRUE, pdb_list, length_dict):
    ...
    for i in range(0, len(PRED[:, 0, 0, 0])):
        ...
        for S in [6, 12, 24]:
            for R in [15]:
                eval_dict[f"Cb-LDDT Radius-Cb-LDDT半径: {R:2d} min-seq-sep: {S:2d}"] = get_LDDT(ND, D, R, S)
